...
/// Registers the three foundation events shared by every faction plugin.
///
/// Add once at app construction with `app.add_plugins(SimEventsPlugin)`.
/// Leaf plugins (units, buildings, infected, …) must **not** call
/// `add_event::<IncomingDamageEvent>()`, `add_event::<EntityKilledEvent>()`, or
/// `add_event::<NoiseEmittedEvent>()` themselves. A second registration of the
/// same event type triggers a Bevy B0001 panic at runtime — one that
/// `cargo build` cannot catch because it only fires when the schedule is first
/// built. Leaf plugins should register only their own faction-local events
/// (e.g. `SpawnSoldierEvent`, `GainSoldierExperienceEvent`).
pub struct SimEventsPlugin;

impl Plugin for SimEventsPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<IncomingDamageEvent>()
            .add_event::<EntityKilledEvent>()
            .add_event::<NoiseEmittedEvent>();
    }
}
